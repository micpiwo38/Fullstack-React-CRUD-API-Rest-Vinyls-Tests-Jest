...
const EditVinyl = ({ vinyl, onVinylUpdated, setDisplayEditForm }) => {
  ...
  //Mettre a jour les données => appelé a chaque chargement d'etat
  useEffect(() => {
  ...
  const saveVinyl = async () => {
    try {
      const response = await axios.put(
        `http://localhost:3001/vinyls/${currentVinyl.id}`,
        currentVinyl,
      );
      if (
        ...
        {vinylFormIsSubmit ? (
          ...
          <div className="container shadow rounded mt-3 p-3">
            <h4 className="text-center text-danger">
              Editer le disque {vinyl.vinyl_name}
            </h4>
            <div className="mt-3">
              <input
                type="text"
                className="form-control"
                placeholder={currentVinyl.vinyl_name}
                required
                name="vinyl_name"
